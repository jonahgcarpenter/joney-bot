...
func TestHardDeleteAllUserDataPreservesAccountAndResetsEverySession(t *testing.T) {
	...
	t.Cleanup(func() { _ = store.Close() })
	seedAccountUsers(t, store, "user", "other")
	if _, err := store.publishFixtureMemory(ctx, "user", memoryFixture{Scope: ScopeLongTerm, Category: "identity", Statement: "The user's name is Ada.", Evidence: "My name is Ada."}); err != nil {
		t.Fatal(err)
	}
	other, err := store.publishFixtureMemory(ctx, "other", memoryFixture{Scope: ScopeLongTerm, Category: "identity", Statement: "The other user's name is Bob.", Evidence: "My name is Bob."})
	if err != nil {
		t.Fatal(err)
	}
	for _, sessionID := range []string{"one", "two"} {
		...
		turn, err := store.appendFixturePendingTurn(ctx, sessionID, "user", profile.Generation, "hello", "hi", nil, time.Hour)
		if err != nil {
			t.Fatal(err)
		}
	...
	sessions, err := store.ResetUserDataPreservingAccount(ctx, "user", time.Now().UTC())
	if err != nil {
		t.Fatal(err)
	}
