...
func userMemoryPatternTool() llm.Tool {
	additionalProperties := false
	minPatterns, maxPatterns := 0, memory.MaxExtractedPatterns
...
func (e *LLMExtractor) ExtractPatterns(ctx context.Context, turns []memory.StoredSessionTurn, previousErrorCode string) (memory.MemoryPatternBatch, error) {
	if len(turns) < 2 || len(turns) > memory.MaxPatternContextTurns {
		return memory.MemoryPatternBatch{}, fmt.Errorf("pattern extraction requires two to eight turns")
	}
	...
	if err != nil {
		return memory.MemoryPatternBatch{}, err
	}
	...
	if err != nil {
		if llm.IsPermanentChatProviderError(err) {
			return memory.MemoryPatternBatch{}, errors.Join(ErrPermanentExtraction, fmt.Errorf("memory pattern extraction: %w", err))
		}
		return memory.MemoryPatternBatch{}, fmt.Errorf("memory pattern extraction: %w", err)
	}
	if resp == nil || len(resp.Message.ToolCalls) == 0 {
		return memory.MemoryPatternBatch{}, invalidOutput(invalidOutputMissingToolCall)
	}
	if len(resp.Message.ToolCalls) != 1 {
		return memory.MemoryPatternBatch{}, invalidOutput(invalidOutputMultipleToolCalls)
	}
	call := resp.Message.ToolCalls[0]
	if call.Function.Name != userMemoryPatternToolName {
		return memory.MemoryPatternBatch{}, invalidOutput(invalidOutputUnexpectedToolCall)
	}
	if _, malformed := call.Function.Arguments["_raw"]; malformed {
		return memory.MemoryPatternBatch{}, invalidOutput(invalidOutputMalformedArguments)
	}
	batch, err := memory.DecodeMemoryPatternBatch(call.Function.Arguments)
	if err != nil {
		return memory.MemoryPatternBatch{}, invalidOutput(invalidOutputBatchShape)
	}
	...
	for _, pattern := range batch.Patterns {
		if !policy.ClaimSlotCompatible(policy.Category(pattern.Category), pattern.ClaimSlot) {
			return memory.MemoryPatternBatch{}, invalidOutput(invalidOutputPatternClaimSlot)
		}
		hasAnchor := false
		for _, observation := range pattern.Observations {
			sourceText, ok := turnText[observation.SourceTurnID]
			if !ok {
				return memory.MemoryPatternBatch{}, invalidOutput(invalidOutputPatternSource)
			}
			if observation.Evidence != sourceText {
				return memory.MemoryPatternBatch{}, invalidOutput(invalidOutputPatternEvidence)
			}
			hasAnchor = hasAnchor || observation.SourceTurnID == anchorID
		}
		if !hasAnchor {
			return memory.MemoryPatternBatch{}, invalidOutput(invalidOutputPatternAnchor)
		}
	}
	return batch, nil
}
...
func (e *LLMExtractor) Extract(ctx context.Context, turn memory.StoredSessionTurn, previousErrorCode string) (memory.MemorySaveBatch, error) {
	if strings.TrimSpace(turn.UserText) == "" {
		return memory.MemorySaveBatch{}, nil
	}
	parallelToolCalls := false
	temperature := 0.0
	...
	if err != nil {
		if llm.IsPermanentChatProviderError(err) {
			return memory.MemorySaveBatch{}, errors.Join(ErrPermanentExtraction, fmt.Errorf("memory formation extraction: %w", err))
		}
		return memory.MemorySaveBatch{}, fmt.Errorf("memory formation extraction: %w", err)
	}
	if resp == nil || len(resp.Message.ToolCalls) == 0 {
		return memory.MemorySaveBatch{}, invalidOutput(invalidOutputMissingToolCall)
	}
	if len(resp.Message.ToolCalls) != 1 {
		return memory.MemorySaveBatch{}, invalidOutput(invalidOutputMultipleToolCalls)
	}
	call := resp.Message.ToolCalls[0]
	if call.Function.Name != userMemorySaveToolName {
		return memory.MemorySaveBatch{}, invalidOutput(invalidOutputUnexpectedToolCall)
	}
	if _, malformed := call.Function.Arguments["_raw"]; malformed {
		return memory.MemorySaveBatch{}, invalidOutput(invalidOutputMalformedArguments)
	}
	batch, itemErrors, err := memory.DecodeMemorySaveBatch(call.Function.Arguments)
	if err != nil {
		return memory.MemorySaveBatch{}, invalidOutput(invalidOutputBatchShape)
	}
	if len(itemErrors) > 0 && len(batch.Memories) == 0 {
		return memory.MemorySaveBatch{}, invalidOutput(invalidOutputAllItemsMalformed)
	}
	return batch, nil
}
...
func validateTool(tool llm.Tool) error {
	...
	item := memories.Items
	if item.Type != "object" || item.AdditionalProperties == nil || *item.AdditionalProperties || len(item.Required) != len(memory.MemorySaveRequiredFields()) {
	...
	for _, field := range memory.MemorySaveRequiredFields() {
