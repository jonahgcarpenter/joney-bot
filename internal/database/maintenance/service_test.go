...
func (s *blockingSweeper) MaintenanceSweep(ctx context.Context, _ time.Time, _ config.RetentionPolicy) (memory.MaintenanceCounts, error) {
	s.mu.Lock()
	s.calls++
	s.active++
	if s.active > 1 {
	...
	s.active--
	s.mu.Unlock()
	return memory.MaintenanceCounts{}, ctx.Err()
}
...
func (s *recoveringSweeper) MaintenanceSweep(context.Context, time.Time, config.RetentionPolicy) (memory.MaintenanceCounts, error) {
	s.count++
	select {
	...
	if s.count == 1 {
		return memory.MaintenanceCounts{}, errors.New("transient cleanup failure")
	}
	return memory.MaintenanceCounts{}, nil
}
